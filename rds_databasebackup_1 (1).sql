...
CREATE PROCEDURE [dbo].[RDS_DatabaseBackup]
...
@FileName nvarchar(max) = '{InstanceName}_{DatabaseName}_{BackupType}_{Partial}_{CopyOnly}_{Year}{Month}{Day}_{Hour}{Minute}{Second}_{FileNumber}.{FileExtension}',
...
BEGIN
  ...
  IF NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'dbo' AND objects.[name] = 'RDS_CommandExecute')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The stored procedure RDS_CommandExecute is missing. Download https://ola.hallengren.com/scripts/RDS_CommandExecute.sql.', 16, 1
  END

  IF EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'dbo' AND objects.[name] = 'RDS_CommandExecute' AND OBJECT_DEFINITION(objects.[object_id]) NOT LIKE '%@DatabaseContext%')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The stored procedure RDS_CommandExecute needs to be updated. Download https://ola.hallengren.com/scripts/RDS_CommandExecute.sql.', 16, 1
  END
  ...
  --IF @AmazonRDS = 1 AND (@S3BucketArn = NULL OR @S3BucketArn = '')
  --BEGIN
  --  INSERT INTO @Errors ([Message], Severity, [State])
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
		    ...
		    EXECUTE @CurrentCommandOutput = dbo.RDS_CommandExecute @DatabaseContext = @CurrentDatabaseContext, @Command = @CurrentCommand, @CommandType = @CurrentCommandType, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
		    SET @Error = @@ERROR
